...
class QKDNetwork:
    ...
    def find_wavelength_allocation(self, wavelength_allocation: np.ndarray) ->int:
        wavelength_allocation_name=[self.wa_subnet1, self.wa_subnet2, self.wa_fullmesh,
                                    self.wa_partial_mesh1, self.wa_partial_mesh2]
        for i in range(len(wavelength_allocation_name)):
            if np.array_equal(wavelength_allocation, wavelength_allocation_name[i]):
                return wavelength_allocation_name[i]
    ...
    effective channel for the specific node 
    ...
    def optimize_network_parameters(self, wavelength_allocation: np.ndarray, num_users:int, wa:int) -> Dict:
        ...
        wrapped_objective = lambda x: -self.multi_secure_key_rate(wavelength_allocation,fiber_length, num_users,
                                                                  self.list_to_matrix(x[:-1]), 1e6)[0]#x[-1]*1000 )[0]
        ...
        best_pgr = 1e6#best_params[n_cw]*1000
        ...
        csv_store = [
            ...
            int(result.nit),  
            bool(result.success),
            str(wavelength_allocation_order)
            ]
    ...
    def plot_dynamic_user(self, dynamic_skr_list):
        ...
        plt.bar(x - bar_width, dynamic_skr_list[0], width=bar_width, label='A', color='blue')
        plt.bar(x, dynamic_skr_list[1], width=bar_width, label='B', color='red')
        plt.bar(x + bar_width, dynamic_skr_list[2], width=bar_width, label='C', color='green')

        # Formatting
        plt.xticks(ticks=x, labels=labels, rotation=45)  # Set x-axis labels and rotate
        plt.xlabel("Names (D)")
        plt.ylabel("Values")
        plt.title("Bar Chart of A, B, and C against D")
        plt.legend()
...
def main():
    ...
    for m in range(6):
        for n in range(6):
            if m<n:
                a,b,c,d=network.num_channel_from_other_users(network.wa_partial_mesh2,m,n)
                print(m,n,a,b,c,d)
    ...
    wavelength_allocation_variable=[network.wa_fullmesh, network.wa_partial_mesh1,
                                    network.wa_partial_mesh2]
    ...
    for m in range(3):
        ...
        dynamic_skr_list.append(network.matrix_to_list(results['secure key rate matrix']))
    ...
    print(dynamic_skr_list)
    
    network.plot_dynamic_user(dynamic_skr_list)
