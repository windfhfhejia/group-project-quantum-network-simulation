...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 27 23:07:01 2025
...
class QKDNetwork:
    ...
    def validate_wavelength_allocation(self,wavelength_allocation)-> tuple[bool, int]: 
        #print(wavelength_allocation)
        sender=[]
        receiver=[]
        ...
        for i in range(len(sender)):
            ...
            if not any(-item in chan2 for item in chan1):
                score=score+10000
            if len(set(chan1)) < len(chan1) or len(set(chan2)) < len(chan2):  
                score=score+10000
        if score==0:
            return True,0
        else:
    ...
    def multi_secure_key_rate(self, wavelength_allocation: np.ndarray,fiber_length:np.ndarray,
        ...
        valid, score=self.validate_wavelength_allocation(wavelength_allocation)
        if valid==False:
            return score, secure_key_rate_matrix, QBER_matrix 
        #print(wavelength_allocation)
    ...
    def list_to_wavelength_allocation(self, num_users: int, wave_list: np.ndarray)-> np.ndarray:
        opt_wavelength_allocation = np.zeros((3, num_users))
        for i in range(len(wave_list)):
            x_rounded = np.round(wave_list[i]).astype(int)
            if x_rounded==0:
                x_rounded = np.sign(np.random.choice([-1, 1]))
            opt_wavelength_allocation[i//num_users][i%num_users]=x_rounded
            
        
        return opt_wavelength_allocation
            
    
    def optimize_network_parameters(self, num_users:int) -> Dict:
        ...
        bounds = [(4, 80)] * num_channel + [(-6,6)]*num_users*3+[(4e2, 1e4)]
        
        
        wrapped_objective = lambda x: -self.multi_secure_key_rate(self.list_to_wavelength_allocation(num_users, x[num_channel:-1]),fiber_length, num_users,
                                                                  self.list_to_matrix(x[0:num_channel])*10, x[-1]*1000 )[0]
        ...
        best_cw_params = best_params[:n_cw]*10
        best_wl=best_params[n_cw:-1]
        best_pgr = best_params[-1]*1000
        ...
        # 计算最终性能
        sum_skr,skr_matrix,qber_matrix = self.multi_secure_key_rate(self.list_to_wavelength_allocation(num_users, best_wl), 
                     fiber_length, num_users, best_cw, best_pgr)
        ...
        csv_store = [
            ...
            [int(param) for param in best_params[:n_cw]],  
    ...
    def check_and_create_csv(self,filename='output3.csv'):
...
def main():
    ...
    for i in range(1):
        network.check_and_create_csv('output3.csv')
       
        
        logging.info("Starting network optimization...")
        results = network.optimize_network_parameters( network.num_users)
